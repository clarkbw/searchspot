...
var GeolocationClass = Class({
  ...
  initialize: function initialize(options) {
    ...
    privateAPI.GeolocationSvc = chrome.Cc["@mozilla.org/geolocation;1"].getService(chrome.Ci.nsIDOMGeoGeolocation);
    ...
    simpleprefs.on(this.ALLOW_GEOLOCATION_PREF, this._onallowed.bind(this), this);
  ...
  get allowed() { return simpleprefs.prefs[this.ALLOW_GEOLOCATION_PREF]; },
  set allowed(allow) { simpleprefs.prefs[this.ALLOW_GEOLOCATION_PREF] = allow; },
  ...
  monitor : function monitor_geolocation() {
    ...
    if (namespace(this).monitorID === null) {
      try {
        ...
        console.log("monitor", this.allowed, namespace(this).monitorID);
  ...
  unload: function geolocation_unload(reason) {
    simpleprefs.removeListener(this.ALLOW_GEOLOCATION_PREF, this._onallowed);
    this._stopmonitor();
