...
SearchSpotPanel.port.on("resize", function(sizes) {
  ...
  var textbox = 300;
  try {
    textbox = ffwm.getActiveWindowDocument().getElementById(SEARCH_TEXTBOX).clientWidth;
  } catch (ignore) { }
...
SearchSpotPanel.port.on("click", function(data) {
  ...
  try {
    ffwm.getActiveWindowDocument().getElementById(SEARCH_TEXTBOX).value = data.terms;
  } catch(ignore) { }
...
exports.main = function (options, callbacks) {
  ...
  function observer(data, subject) {
    // This is the removal of a non-default installed engine, defaults are "changed"
    if ("engine-removed" == subject) {
      try {
        SearchSpotPanel.port.emit("removeEngine", objectifyEngine(data.QueryInterface(Ci.nsISearchEngine)));
      } catch (err) { dump("error removing engine: " + err + "\n"); }
    } else if ("engine-added" == subject) {
        SearchSpotPanel.port.emit("addEngine", objectifyEngine(data.QueryInterface(Ci.nsISearchEngine)));
    } else if ("engine-changed" == subject) {
        var engine = data.QueryInterface(Ci.nsISearchEngine);
        // removing a default engine only actually hides it, they are not removed
        if (engine.hidden) {
          try {
            SearchSpotPanel.port.emit("removeEngine", objectifyEngine(engine));
          } catch (err) { dump("error hiding / removing engine: " + err + "\n"); }
        }
      //dump("name: " + engine.name + "\n");
      //dump("description: " + engine.description + "\n");

    // This sets the current engine in use, a concept that doesn't mean much in our world just yet
  ...
  obs.add("browser-search-engine-modified",observer);
  require("unload").when(function () { obs.remove("browser-search-engine-modified", observer); });

  // Do some geolocation magic and add Yelp to our Search Engines once we have it
  getLocation();

};

function objectifyEngine(engine) {
  var spec = (engine.iconURI)? engine.iconURI.spec : "";
...
function setEngines() {
  ...
  for each (let engine in Services.search.getVisibleEngines()) {
    //dump("name: " + engine.name + "\n");
    engines.push(objectifyEngine(engine));
  }
  SearchSpotPanel.port.emit("setEngines", engines);
  gEnginesInit = true;
}
  ...
  if (textbox) {
    ...
    var openpanel = function(e) {
      ...
      if (!SearchSpotPanel.isShowing) {
        ...
        // This should only initialize the engines once
        setEngines();

      } else {
        // Set the terms before we allow them to hit enter
        SearchSpotPanel.port.emit("setTerms",textbox.value);
                  ...
                  if (request.status == 200) {
                    ...
                    for (var i in suggestions) {
                      ...
                      if (terms !=  suggestions[i]) {
                        results.push({ "title" : suggestions[i], "url" : (type == "match")? baseurl + suggestions[i] : "" });
                      }
                    }
