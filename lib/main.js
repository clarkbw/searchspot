const data = require("self").data,
      ...
      { Geolocation : geolocation } = require("geolocation"),
...
BrowserSearchEngines.on("current", function(engine) {
  // we don't really care about the current engine right now
});
...
SearchSpotPanel.port.on("click", function(data) {
  ...
  let location = "";
  try { location = geolocation.address.city + ", " + geolocation.address.region; } catch(ignore) {}
...
exports.main = function (options, callbacks) {
  ...
  geolocation.once("position", function() {
    // Add Yelp to our Search Engines once we have geolocation
    //console.log("Adding Yelp");
      ...
      try {
        function refreshSuggestions()
        {
          ...
          for each (let engine in BrowserSearchEngines.getVisible()) {
            ...
            let location = "";
            try { location = geolocation.address.city + ", " + geolocation.address.region; } catch(ignore) {}
            let suggestions = engine.getSuggestion(textbox.value, location);
