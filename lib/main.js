...
var data = require('sdk/self').data,
    ...
    { addXULStylesheet, setLogLevel } = require('utils');
...
// Only for testing purposes do we export this
exports.SearchSpotPanel = SearchSpotPanel;
...
SearchSpotPanel.port.on("click", function (results) {
  ...
  console.debug("click", engine, terms, url, results);
  ...
  tabs.activeTab.once('close', function () {
...
SearchSuggestManager.on("suggestions", function (engine, terms, results) {
  if (searchbar.getSearchTextBox().value === terms) {
    console.debug("change:suggestions", engine, terms, results);
    SearchSpotPanel.port.emit("suggestions", engine, terms, results);
  }
});

SearchSuggestManager.on("change:terms", function (terms) {
  console.debug("change:terms", terms);
  SearchSpotPanel.port.emit("terms.reset", terms);
});

function closepanel() {
  setTimeout(function () {
    if (SearchSpotPanel && SearchSpotPanel.isShowing &&
        searchbar && !searchbar.getSearchTextBox().textbox.focused) {
      SearchSpotPanel.hide();
    }
  }, 500);
}
...
exports.main = function (options) {
  ...
  addXULStylesheet(data.url("searchspot-style.css"));
