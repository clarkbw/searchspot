...
var data = require('sdk/self').data,
    WindowTracker = require('window-utils').WindowTracker,
...
exports.main = function (options, callbacks) {
  options = options || { loadReason : "no options" };
  // in our test environment we want to ensure that we can
  // simulate an 'install' event
  if (options.staticArgs && options.staticArgs.loadReason) {
    options.loadReason = options.staticArgs.loadReason;
  }

  require("unload").ensure(new WindowTracker(new WindowManager()));
  ...
  if (options.loadReason === "install" || options.loadReason === "upgrade") {
    require("tabs").open(data.url("index.html#welcome"));
  }
  // Debug notifications
