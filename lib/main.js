...
var data = require("self").data,
    ...
    isFirefox = require("sdk/system/xul-app").is("Firefox"),
    ...
    onTrack: function ffWindowManager_onTrack(window) {
      if (isBrowser(window)) {
        if (isFirefox) {
          ...
          var page = "http://clarkbw.github.com/searchspot/" +
          // var page = "http://people.mozilla.com/~bwinton/search_suggest/" +
                      simpleprefs.prefs.version + "/fennec.html";
          window.document.defaultView.NativeWindow.setSearchPage({
            page: page,
            onTerms: function addon_onTerms(terms) {
              SearchSuggestManager.search(terms);
            },
            onClick: function addon_onClick(args) {
              console.error("Got Click!  " + JSON.parse(args));
              args = JSON.parse(args);
              var terms = args.pop(),
                  engine = SearchEngines.get(args.pop()),
                  url = engine.getSubmission(terms) || "about:home";

              // either of these needs to close our search thing and show the page
              // tabs.activeTab.url = url;
              tabs.open({ url : url });
            }
          });
          SearchSuggestManager.on("suggestions", function (engine, terms, results) {
            try {
              window.document.defaultView.NativeWindow.onSuggestions({ "engine" : engine.toJSON(),
                                                                       "terms" : terms,
                                                                       "results" : results });
            } catch (e) {
              console.error(e);
    ...
    onUntrack: function ffWindowManager_onUntrack(window) {
      if (isBrowser(window)) {
        if (isFirefox) {
          ...
          SearchSuggestManager.off("suggestions");
        }
      }
    }
  };
