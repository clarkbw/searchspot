...
function getSearchTextBox() {
  return windowMediator.getMostRecentWindow("mail:3pane").document.getElementById(SEARCH_TEXTBOX);
...
SearchSpotPanel.port.on("click", function(data) {
  ...
  windowMediator.getMostRecentWindow("mail:3pane").window.openTab("contentTab", { "contentPage" : url });
...
function ffWindowManager() {
  return {
    ...
    isMail : function ffWindowManager_isMail(window) {
...
function attachToSearch(document) {
  ...
  if (textbox) {
    ...
    searchbox.onSearchBegin = function(e) { }
    searchbox.onSearchComplete = function(e) { }
