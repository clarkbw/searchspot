...
SearchEngines.on("suggestions", function(type, data) {
  console.log("on.suggestions", type, data.terms, data.name, data.id, data.type);
  if (getSearchTextBox().value == data.terms) {
    SearchSpotPanel.port.emit(type, data);
  }
});

SearchEngines.on("removed", function(engine) {
  SearchSpotPanel.port.emit("removeEngine", engine);
});

SearchEngines.on("added", function(engine) {
...
SearchSpotPanel.port.on("click", function(data) {
  let engine = SearchEngines.get(data.engine);
  console.log("click", data.engine, engine, data.terms);
  ...
  tabs.activeTab.url = engine.getSubmission(data.terms);
