...
const data = require("self").data,
      ...
      { AwesomeBarSuggestions } = require("awesomebar-suggestions"),
...
var SearchSpotPanel = require("autocomplete-panel").Panel({
  ...
  contentScriptFile : [data.url("js/jquery.js"),
                       data.url("results.js")],
  onShow : function() {
    SearchSpotPanel.port.emit("setEngines", SearchEngines.engines)
...
SearchSpotPanel.port.on("click", function(data) {
  ...
  StatisticsReporter.send("use", { "engine" : SearchEngines.get(data.id), "stats" : data.stats });

  url = SearchEngines.getSubmission(data.id, data.terms),
  terms = data.terms;
...
exports.main = function (options, callbacks) {
  ...
  if (options.staticArgs.dev) {
    SearchEngines.debug(true);
    StatisticsReporter.debug(true);
  }
};
