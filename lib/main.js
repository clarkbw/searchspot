...
var SearchSpotPanel = require("autocomplete-panel").Panel({
  ...
  onShow : function() {
    ...
    SearchSpotPanel.port.emit("terms.reset", getSearchTextBox().value);
  }
});
...
SearchSpotPanel.port.on("click", function(data) {
  ...
  winUtils.activeBrowserWindow.focus()
...
function openpanel(e) {
  ...
  SearchSpotPanel.port.emit("terms.reset",searchbox.value);
...
function _findStylesheet(document) {
  var uri = data.url(STYLESHEET_ID + ".css"),
      ...
      ORDERED_NODE_SNAPSHOT_TYPE = 7, // XPathResult doesn't exist in this context
      xpath = null;
  ...
  for (var i = 0; xpath && i < xpath.snapshotLength; i++) {
...
function addStylesheet(document) {
  var { item , css } = _findStylesheet(document);
  if (item === null) {
    document.insertBefore(document.createProcessingInstruction("xml-stylesheet", css),
                          document.firstChild);
  }
}

function removeStylesheet(document) {
  var { item, _ } = _findStylesheet(document),
      parent = null;
  if (item !== null) {
