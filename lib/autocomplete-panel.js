...
const Panel = Symbiont.resolve({
  ...
  show: function show(anchor) {
    ...
    if (!xulPanel) {
      ...
      let css = ".searchspot-panel, .panel-inner-arrowcontent, .panel-arrowcontent {padding: 0;}";
  ...
  _onShow: function _onShow() {
    try {
      if (!this._inited) { // defer if not initialized yet
        ...
        // XXX we don't want them mucking with our HTML
...
function getWindow(anchor) {
  ...
  if (!window)
    window = windowMediator.getMostRecentWindow("mail:3pane");
