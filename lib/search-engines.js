  ...
  get defaults() {
    ...
    return {
        ...
        if (engine.usesGeoLocation) {
          ...
          GeoPermissionPanel.port.once("click", function click(data) {
            ...
            if (data == "ok") {
              ...
              // try to add this to the others list in case it doesn't already exist there
      ...
      sort : function sort(newOrder) {
        ...
        // Send out an event that there is a new order in town.  Regulators! Mount Up!
        self._emit("defaults.sorted", self._engines.map(function(engine) { return engine.id; }));
      },
...
const SearchSuggestManager = EventEmitter.compose({
  _emit: EventEmitter.required,
  on: EventEmitter.required,
  ...
  _run : function (terms) {
    ...
    for (var i = 0; i < SearchEngines.defaults.all.length; i++) {
      var engine = SearchEngines.defaults.all[i];
