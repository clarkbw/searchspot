...
var SearchEngines = EventEmitter.compose({
  ...
  _first_run : function _first_run() {
    ...
    this.defaults.add(new SearchEngine("http://www.yelp.com/opensearch",
                                     "Yelp",
                                     ...
                                     "http://s3-media4.ak.yelpcdn.com/assets/2/www/img/1273e7149fc0/ico/favicon_multi.ico"), true);

    // Provide a mapping of the OpenSearch descriptors to our default engines
    // (which either don't have them or are incorrect)
    var sites   = {
      "Wikipedia (en)" : "http://en.wikipedia.org/wiki/Special:Search",
    ...
    return {
      ...
      add : function add(engine) {
        ...
        if (engine.usesGeoLocation) {
          ...
          // For mobile, just add the engine and turn on geolocation.
          if (!require("sdk/system/xul-app").is("Firefox")) {
            actuallyAdd(engine);
    ...
    this.request.onreadystatechange = function (aEvt) {
      if (this.request.readyState === 4) {
        if (this.request.status === this.HTTP_OK) {
          ...
          console.error('req.error', this.engine.name,
