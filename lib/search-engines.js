...
const SearchEngines = EventEmitter.compose({
  ...
  // This is receiving <link> elements from the page-mod
  ...
  addByURL : function addByURL(url) {
    ...
    console.log("addByURL.open", url);
    request.onreadystatechange = function (aEvt) {
      if (request.readyState == 4) {
        if (request.status == 200) {
          if (request.responseXML) {
            var parsed = this._parse(request.responseXML);
            let engine = SearchEngine(url, parsed.name, parsed.queryURL, parsed.suggestionURL, parsed.icon);
            this.add(engine, [DEFAULT_TAG, FOUND_TAG]);
          }
        }
      }
    }.bind(this);
...
require("page-mod").PageMod({
  ...
  contentScriptFile: data.url("search-engine-pagemod.js"),
  onAttach: function(worker) {
    worker.on('message', function(data) {
      console.log("PageMod", data);
      SearchEngines.collect(data);
    });
  }
});
