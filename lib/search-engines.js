...
var Class = require('sdk/core/heritage').Class,
    ...
    DatabaseFactory = require('indexed-db-storage').DatabaseFactory,
...
var SearchEngines = Class({
  ...
  _debug : function _debug(isDebug) {
    if (isDebug) {
      console.log("SearchEngines debug");
    }
  },
  ...
  initialize : function initialize() {
    ...
    DatabaseFactory.open('awesome-search').then(function (db) {
      privateAPI.db = db;
      console.log('private DB', db);
      if (db.objectStoreNames.indexOf('engines') < 0) {
        console.log('object store does not exist');
        db.createObjectStore('engines', { 'keyPath' : 'name' }).then(function (objectStore) {
          console.log('created object store', objectStore);
          privateAPI.objectStore = objectStore;
          self._first_run();
        });
      }
    });

    // XXX keep this around for backwards compat
    if (!storage.engines) {
    ...
    SystemEvents.on("search:debug", this._debug.bind(this));
  ...
  _first_run : function _first_run() {
    ...
    emit(this, 'initialized', namespace(this).objectStore);

    // set our intial default sort order
    // this.defaults.sort(order);
    console.log("ALL", namespace(this).objectStore.all());
  },
  ...
  get defaults() {
    ...
    return {
      remove : function remove(engine) {
        ...
        var index = -1,
            result = null;
        privateAPI.defaults.every(function (e, i) {
          if (engine.equals(e)) {
            index = i;
            return false;
          }
        ...
        if (index >= 0) {
          ...
          if (result) {
            privateAPI.objectStore.update(engine.name, { 'isDefault' : false }).then(function (engine) {
      ...
      add : function add(engine) {
        ...
        if (privateAPI.defaults.some(function (e) { return engine.equals(e); })) {
          return engine;
        }

        // Add the engine
        privateAPI.defaults.push(engine);

        privateAPI.objectStore.get(engine.name).then(function (e) {
          console.log("GET", e);
          if (e) {
            // XXX do an updating routine here
            e.isDefault = true;
          } else {
            console.log("adding", engine.name);
            privateAPI.objectStore.add(engine, engine.name).then(function (addedEngine) {
              console.log("UPDATED", addedEngine);
              // Send out the add event
              emit(self, "defaults.added", addedEngine);
            });
          }
        }, function newEngine(newEngine) {
          console.log("newEngine", newEngine);
          privateAPI.objectStore.add(engine, engine.name).then(function (engine) {
            console.log("NEW ADDED", engine);
            // Send out the add event
  ...
  _update : function _update(engine) {
    ...
    if (namespace(this).engines.some(function (e, i, a) {
                              if (e.id === engine.id) {
                                e = engine;
                                return true;
                              }
  ...
  unload : function unload(reason) {
    ...
    SystemEvents.off("search:debug", this._debug);
...
var SearchSuggestManager = Class({
  ...
  initialize : function initialize() {
    ...
    SearchEngines.on('initialized', function () { this._set_engines(); }.bind(this) );
