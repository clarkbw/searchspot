...
const SearchEngines = EventEmitter.compose({
  ...
  _first_run : function _first_run() {
    ...
    BrowserSearchEngines.getVisible().forEach(function (engine, i, a) {
      var queryURL = decodeURIComponent(engine.getSubmission("{searchTerms}", "{searchLocation}"));
      var suggestionURL = decodeURIComponent(engine.getSuggestion("{searchTerms}", "{searchLocation}") || "");
      ...
      this.add(SearchEngine(site, engine.name, queryURL, suggestionURL, engine.icon, type, base),
               tags[engine.name] || [DEFAULT_TAG, FOUND_TAG]);
  ...
  _collector : function _collector(engine) {
    ...
    // if this engine already exists lets just check that it hasn't been updated
    if (se) {
      if (se.queryURL == engine.queryURL &&
  ...
  getSubmission : function (id, terms) {
    ...
    //console.log("getSubmission", id, this.get(id), terms, location);
