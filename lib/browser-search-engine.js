...
const SearchEngine = Trait.compose({
  ...
  addSuggest: function addSuggest(url) {
    ...
    // XXX Current versions of Firefox display an alert to the user when you try
    //     to use addParam() on a read-only engine.  We'll have to just ignore official
    //     use from now on and use our own mapping.  Thanks Firefox!
    return;
