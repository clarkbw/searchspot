...
const SearchEnginesCollector = EventEmitter.compose({
  ...
  constructor : function SearchEnginesCollector() {
    // Debug listener
    this.on("href", function(host, href) {
      console.log("host", host, "href", href);
    });

    // Debug listener
    this.on("engine", function(engine) {
      console.log(engine);
    });
  },

  // This is receiving <link> elements from the PageMod
  // expects parameters of [{ site : document.URL, name : <link title>, opensearch : <link href> }]
  ...
  // retrieve an engine via the XML url parameter
  // emits "engine" when the engine has been found and parsed
  ...
  // Lightweight parsing function for an XML doc representing an OpenSearch document
  // returns a SearchEngine object
...
// PageMod that searches all pages for these link references
 //<link rel="search"
