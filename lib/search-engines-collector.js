...
const SearchEnginesCollector = EventEmitter.compose({
  ...
  _parse : function _parse(url, doc) {
    ...
    for (var i = 0; i < urls.length; i++) {
      ...
      var params = urls.item(i).getElementsByTagName("Param");
      // XXX I'm not really proud of this
      template += (template.indexOf("?") >=0)? "" : "?";
      for (var j = 0; j < params.length; j++) {
        if (template[template.length - 1] != "?" && template[template.length - 1] != "&") {
          template += "&";
        }
        template += params.item(i).getAttribute("name") + "=" + params.item(i).getAttribute("value");
      }
    ...
    try {
      ...
      opensearch["icon"] = doc.getElementsByTagName("Image")[0].textContent;
