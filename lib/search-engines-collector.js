...
var EventEmitter = require("events").EventEmitter,
    ...
    fetchImageDataSync = require("utils").fetchImageDataSync;
...
var SearchEnginesCollector = EventEmitter.compose({
  ...
  _parse : function _parse(url, doc) {
    ...
    try {
      var icon = doc.getElementsByTagName(TAG_IMAGE).item(0).textContent;
      var href = URL.URL(icon);
      opensearch.icon = fetchImageDataSync(href.toString());
    } catch (noicon) {
