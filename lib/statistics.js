...
const StatisticsReporter = EventEmitter.compose({
  ...
  // The intended host name is "https://search.vcap.mozillalabs.com/service"
  // However until the certificate for this domain name is valid we are using
  // a temporary search-vcap hostname which carries a valid certificate
  ...
  // Call this function to get a debug mode where the timeout is short and the
  // statistics are sent only to the localhost
  debug : function(isDebug) {
    if (isDebug) {
      this._url = "http://127.0.0.1:8080/service";
      this._timeout = ONE_SECOND * 5;
      // reset the timer with the above changes
      this._setTimer();
    }
  },
  ...
  _run : function () {
    ...
    function(req) {
      ...
      try {
        this._stats = this._stats.concat(item);
