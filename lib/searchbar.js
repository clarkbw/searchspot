...
function attachToSearch(document, openpanel, closepanel) {
  if (!require("sdk/system/xul-app").is("Firefox")) {
    document.defaultView.NativeWindow.setSearchPage(openpanel, function(terms) {
      // Call back into the NativeWindow.
      document.defaultView.NativeWindow.onSuggestions("Suggestions for " + terms);
    });
    return;
  }
